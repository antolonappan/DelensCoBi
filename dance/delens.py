...
class Delens:
    ...
    def Btemp(self,i,th=True):
        fname = os.path.join(self.basedir,f"bmode_temp_{'g'if th else 'r'}{'' if self.lensed else 'gaus' }_{i:04d}.fits")
    ...
    def delens_cl(self,i,th=True):
        fname = os.path.join(self.basedir,f"delens_cl_{'g'if th else 'r'}{'' if self.lensed else 'gaus' }_{i:04d}.pkl")
        if os.path.isfile(fname):
            return pl.load(open(fname,'rb'))
        else:
            e = self.wf.mysims.sky.get_E(i)
            b = self.wf.mysims.sky.get_B(i)
            btemp = self.Btemp(i,th)
    ...
    def get_data(self,debias:bool = False):
